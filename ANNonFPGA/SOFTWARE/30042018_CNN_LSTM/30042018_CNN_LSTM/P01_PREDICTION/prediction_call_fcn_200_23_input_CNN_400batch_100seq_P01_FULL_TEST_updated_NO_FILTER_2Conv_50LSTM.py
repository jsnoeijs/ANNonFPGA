...
lowcut = 0.5
highcut = 29
X = butter_bandpass_filter(X[:,:,:,:,:,:], lowcut, highcut, fs, order=8)
...
model.fit(X_new,Y_new, epochs=30)
#Trying model
yhat = model.predict(X_new, verbose=0)
file1 = open("CNN_LSTM_dataset_train_100seq_FILTER_3Conv.txt", 'ab')
...
with open("final_data_train_100seq_FILTER_3Conv.txt", 'w') as file_1:
 for i in range(0,a):
    #file_1.write(' '+str(i)+' ')
...
file2 = open("CNN_LSTM_400batch_50_ouputs_100seq_FILTER_3Conv.txt", 'ab')
...
with open("final_data_FILTER_3Conv.txt", 'w') as file_1:
 for i in range(0,a):
    #file_1.write(' '+str(i)+' ')
...
output_count(seq_number, test_files, yhat_test, 'updated_100seq_NO_FILTER_30epochs_2Conv_50LSTM')
