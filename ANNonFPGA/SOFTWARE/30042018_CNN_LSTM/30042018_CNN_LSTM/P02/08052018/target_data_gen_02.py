...
def get_sizes_02(X, dataset_size):
    f = pyedflib.EdfReader("../../dataset_02/chb02_{}.edf".format(1))
    n = f.signals_in_file
    input_size = n
    ...
    for j in range(2,dataset_size+1):
        if (j == 19 or j==28 or j==35 or j==44 or j==45):
            j+=1
        print(("Reading:dataset_01/chb02_{}.edf ...".format(j)))
        f = pyedflib.EdfReader("../../dataset_02/chb02_{}.edf".format(j))
        for i in np.arange(n):
            sigbufs[i,:] = f.readSignal(i)
        sigbufs_test = sigbufs
...
def target_gen_02(output, batch_num, dataset_size, batch_size, file):
